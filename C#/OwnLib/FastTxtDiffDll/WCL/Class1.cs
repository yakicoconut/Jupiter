 ...
 *   参考
 *     [きつねのしっぽ] - SiroKuro Page
 *     	http://d.hatena.ne.jp/SiroKuro/searchdiary?word=%2A%5B%A4%AD%A4%C4%A4%CD%A4%CE%A4%B7%A4%C3%A4%DD%5D
 * 
 */
#endregion
namespace FastTxtDiff
{
  ...
  public class FastDiff
  {
    ...
    #region ☆山内:デフォルトコンストラクタ

    /// <summary>
    /// ☆山内:デフォルトコンストラクタ
    /// ☆山内:メンバが静的のためプライベート設定
    /// </summary>
    private FastDiff() { }
    ...
    #region ☆山内:文字コード数値変換静的メソッド_比較元先引数
    ...
    public class Test
    {
      ...
      /// <returns></returns>
      public static bool TestSplitChar(string text, DiffOption option)
      {
        // 文字コード数値変換静的メソッド使用
        SplitChar(text, option);

        return true;
      }
      #endregion

      #region テスト002_SplitLineテスト
      /// <summary>
      /// SplitLineテスト
      /// </summary>
      /// <param name="text"></param>
      /// <param name="option"></param>
      /// <returns></returns>
      public static bool TestSplitLine(string text, DiffOption option)
      {
        // 行単位分離静的メソッド使用
        SplitLine(text, option);
